  ...
  baseModule = {config, ...}: {
    ...
    in
      mkBefore ''
        ...
        ${
          if pkgs.lib.isFunction test-helpers.extraTests
          then test-helpers.extraTests args
          else test-helpers.extraTests
        }
      '';
  };

  genTests = version: let
    testBase.imports = [
      baseModule
      {
