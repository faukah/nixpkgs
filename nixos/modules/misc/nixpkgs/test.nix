...
in
  lib.recurseIntoAttrs {
    ...
    assertions = assert withHost._module.args.pkgs.stdenv.hostPlatform.system == "aarch64-linux";
    ...
    assert withSameHostAndBuild.config.nixpkgs.buildPlatform
    == withSameHostAndBuild.config.nixpkgs.hostPlatform;
    assert withSameHostAndBuild._module.args.pkgs.stdenv.buildPlatform
    ...
    assert builtins.trace (lib.head (getErrors externalPkgsWithConfig)) getErrors externalPkgsWithConfig
    == [
      ''
