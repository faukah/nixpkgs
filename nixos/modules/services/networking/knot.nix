{
  ...
  yamlConfig = let
    ...
    in
      if secsBad == []
      then true
      else throw ("services.knot.settings contains unknown sections: " + toString secsBad);
    ...
    sec_plain = nix_def: sec_name:
      if !hasAttr sec_name nix_def
      then ""
      else n2y "" {${sec_name} = nix_def.${sec_name};};
    ...
    sec_list_fa = fa_name: nix_def: sec_name:
      if !hasAttr sec_name nix_def
      then ""
      else let
      ...
      in
        ...
        + (
          if isList sec
          then flip concatMapStrings sec (elem: elem2yaml elem.${fa_name} (removeAttrs elem [fa_name]))
    ...
    n2y = indent: val:
      ...
      then
        ...
        */
        if isList val # and long indent
        then "[ " + concatMapStringsSep ", " quoteString val + " ]"
        else if isBool val
        then
          (
            if val
  ...
  configFile =
    ...
    then
      ...
      assert cfg.settings == {} && (cfg.keyFiles == [] || cfg.extraConfig != null);
        cfg.settingsFile
    else mkConfigFile yamlConfig;

  mkConfigFile = configString:
    pkgs.writeTextFile {
      name = "knot.conf";
