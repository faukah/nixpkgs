{
  ...
  toScopedUsers = listenerScope: users:
    lib.pipe users [
      lib.attrNames
      (lib.imap0 (
  ...
  authPluginOptions = with lib.types;
    submodule {
      options = {
        plugin = lib.mkOption {
          type = path;
  ...
  listenerOptions = with lib.types;
    submodule {
      options = {
        port = lib.mkOption {
          type = port;
