...
in {
  ...
  config = lib.mkIf (eachBlockbook != {}) {
    systemd.services =
      lib.mapAttrs' (
        blockbookName: cfg: (lib.nameValuePair "blockbook-frontend-${blockbookName}" (
          let
            configFile =
          ...
          in {
            ...
            serviceConfig = {
              ...
              ExecStart = ''
                ...
                ${
                  if (cfg.rpc.passwordFile != null && cfg.configFile == null)
                  then "-blockchaincfg=${cfg.dataDir}/${blockbookName}-config.json"
                  else "-blockchaincfg=${configFile}"
                } \
                -datadir=${cfg.dataDir} \
