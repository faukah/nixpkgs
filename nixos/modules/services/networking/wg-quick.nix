...
with lib; let
  ...
  generateUnit = name: values:
    ...
    assert assertMsg (
    ...
    ) "generatePrivateKeyFile requires privateKeyFile to be set"; let
      wgBin =
        {
          wireguard = "wg";
          amneziawg = "awg";
      ...
      generateKeyScriptFile =
        if values.generatePrivateKeyFile
        then writeScriptFile "generatePrivateKey.sh" (generatePrivateKeyScript values.privateKeyFile wgBin)
        else null;
      preUpFile =
        if values.preUp != ""
        then writeScriptFile "preUp.sh" values.preUp
        else null;
      postUp =
        optional (
      ...
      postUpFile =
        if postUp != []
        then writeScriptFile "postUp.sh" (concatMapStringsSep "\n" (line: line) postUp)
        else null;
      preDownFile =
        if values.preDown != ""
        then writeScriptFile "preDown.sh" values.preDown
        else null;
      postDownFile =
        if values.postDown != ""
        then writeScriptFile "postDown.sh" values.postDown
        else null;
