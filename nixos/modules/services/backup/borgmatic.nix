        ...
        map (
          d: let
            as_user =
              ...
              then "${pkgs.sudo}/bin/sudo -u ${d.username} "
              else "";
          in
            {
              pg_dump_command =
                if d.name == "all" && (!(d ? format) || isNull d.format)
                then "${as_user}${postgresql}/bin/pg_dumpall"
                else "${as_user}${postgresql}/bin/pg_dump";
...
in {
  options.services.borgmatic = {
    ...
    enableConfigCheck =
      lib.mkEnableOption "checking all configurations during build time"
      // {
        default = true;
      };
  };
