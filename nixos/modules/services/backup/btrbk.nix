{
  ...
  sudo_doas =
    if config.security.sudo.enable || config.security.sudo-rs.enable
    then "sudo"
    else if config.security.doas.enable
    then "doas"
    ...
    users.users.btrbk = {
      ...
      openssh.authorizedKeys.keys =
        map (
          ...
          in ''command="${pkgs.util-linux}/bin/ionice -t -c ${toString ioniceClass} ${
