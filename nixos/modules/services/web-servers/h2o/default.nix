...
let
  ...
  getNames = name: vhostSettings: rec {
    server =
      if vhostSettings.serverName != null
      then vhostSettings.serverName
      else name;
    cert =
      if lib.attrByPath ["acme" "useHost"] null vhostSettings == null
      then server
      else vhostSettings.acme.useHost;
  };
  ...
  acmeEnabledHostsConfigs =
    lib.foldlAttrs (
      acc: name: value:
        if value.acme == null || (!value.acme.enable && value.acme.useHost == null)
        then acc
        else let
  ...
  acmeCertNames = let
    partition = acc: vhostSettings: let
      inherit (vhostSettings) certName;
      isDependent = certs.${certName}.dnsProvider == null;
    in
      if isDependent && !(builtins.elem certName acc.dependent)
      then acc // {dependent = acc.dependent ++ [certName];}
      else if !isDependent && !(builtins.elem certName acc.independent)
      then acc // {independent = acc.independent ++ [certName];}
      else acc;
  ...
  mozTLSRecs =
    if cfg.defaultTLSRecommendations != null
    then let
    ...
    lib.concatMapAttrs (
      name: value: let
        ...
        tlsSettings =
          ...
          {
            "${names.server}:${builtins.toString port.TLS}" = let
              ...
              headerRecAttrs =
                ...
                (
                  ...
                  in {
                    "header.set" =
                      if builtins.isString headerSet
                      then [
                        headerSet
                        hsts
                      ]
                ...
                baseListen =
                  {
                    port = port.TLS;
                    ssl =
                      (lib.recursiveUpdate tlsRecAttrs value.tls.extraSettings)
                      // {
                        inherit identity;
                      };
                  }
...
in {
  ...
  config = mkIf cfg.enable {
    ...
    # of certs end-to-end.
    systemd.services.h2o-config-reload = let
    ...
    security.acme.certs = let
      mkCerts = acc: vhostSettings:
        if vhostSettings.acme.useHost == null
        then let
