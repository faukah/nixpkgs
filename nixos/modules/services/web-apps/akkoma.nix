  ...
  initDbScript = writeShell {
    ...
    text = ''
      ...
      ${optionalString (pgpass != null) ''
        ...
        replace-secret ${
          escapeShellArgs [
            (sha256 cfg.initDb.password._secret)
            cfg.initDb.password._secret
          ]
        } "$pgpass"
...
in {
            ...
            ":pleroma" = {
              ...
              in {
                base_url = mkOption {
                  type = types.nonEmptyStr;
                  default =
                    if versionOlder config.system.stateVersion "24.05"
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.akkoma-initdb = mkIf cfg.initDb.enable {
      ...
      serviceConfig = {
        Type = "oneshot";
        User = mkIf (db ? socket_dir || db ? socket) cfg.initDb.username;
        RemainAfterExit = true;
        UMask = "0077";
