...
with lib; let
  ...
  in
    ...
    else if isList v && strings.isConvertibleWithToString v
    then toPhpString (concatMapStringsSep "," toString v)
    else if isInt v
    then toString v
    else if isBool v
    then
    ...
    then "trim(file_get_contents(${toPhpString (toString v._file)}))"
  ...
  mkPhpKeyVal = k: v: let
    values =
      if (isAttrs v && (hasAttr "_file" v || hasAttr "_raw" v)) || !isAttrs v
      then [" = ${mkPhpValue v};"]
  ...
  pkg = hostName: cfg:
    cfg.package.combine {
      ...
      aclConfig =
        if cfg.settings.useacl && cfg.acl != null
        then dokuwikiAclAuthConfig hostName cfg
        else null;
    };
  ...
  siteOpts = {
    ...
    options = {
      ...
      aclFile = mkOption {
        ...
        default =
          if (config.mergedConfig.useacl && config.acl == null)
          then "/var/lib/dokuwiki/${name}/acl.auth.php"
          else null;
        description = ''
      ...
      usersFile = mkOption {
        type = with types; nullOr str;
        default =
          if config.mergedConfig.useacl
          then "/var/lib/dokuwiki/${name}/users.auth.php"
          else null;
        description = ''
  ...
  config = mkIf (eachSite != {}) (mkMerge [
    {
      ...
      services.phpfpm.pools =
        mapAttrs' (
          hostName: cfg: (nameValuePair "dokuwiki-${hostName}" {
            inherit user;
    ...
    (mkIf (cfg.webserver == "caddy") {
      services.caddy = {
        ...
        virtualHosts =
          mapAttrs' (
            hostName: cfg: (nameValuePair hostName {
              extraConfig = ''
                root * ${pkg hostName cfg}/share/dokuwiki
                file_server
