...
with lib; let
  ...
  listenerSupportsResource = resource: listener: lib.any ({names, ...}: builtins.elem resource names) listener.resources;
...
in {
  ...
  in {
    services.matrix-synapse = {
      ...
      extras = mkOption {
        type = types.listOf (
          types.enum (lib.attrNames pkgs.matrix-synapse-unwrapped.optional-dependencies)
        );
          ...
          assertion = let
            ...
            listener =
              lib.findFirst (
                listener:
                  (
                    lib.hasAttr "port" main
                    && listener.port or null == main.port
