{
  ...
  generateConfigName = name: builtins.replaceStrings ["/"] ["-"] (lib.strings.removePrefix "/" name);
  configFiles =
    builtins.mapAttrs (
  ...
  getMountDeps = settings: builtins.concatStringsSep " " (builtins.catAttrs "Path" (builtins.attrValues settings));
...
in {
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.tmpfiles.rules =
      (map (path: "d ${path} 0770 - autotier - -") mountPaths)
      ++ [
