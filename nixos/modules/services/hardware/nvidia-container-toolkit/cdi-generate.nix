{
  ...
  mountToCommand = mount: "additionalMount \"${mount.hostPath}\" \"${mount.containerPath}\" '${builtins.toJSON mount.mountOptions}'";
  mountsToCommands = mounts:
    if (builtins.length mounts) == 0
    then "cat"
  ...
  writeScriptBin "nvidia-cdi-generator" ''
    ...
    function cdiGenerate {
      ${lib.getExe' nvidia-container-toolkit "nvidia-ctk"} cdi generate \
        --format json \
        ${
      if (builtins.length csv-files) > 0
