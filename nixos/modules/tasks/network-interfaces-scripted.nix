...
with lib; let
  ...
  bondWarnings = let
    ...
    bondText = bondName: optName: _: "${bondName}.${optName} is deprecated, use ${bondName}.driverOptions";
  in {
        ...
        nameValuePair "${n}-fou-encap" (
          let
            ...
            fouSpec = "port ${toString v.port} ${
              if v.protocol != null
              then "ipproto ${toString v.protocol}"
              else "gue"
            } ${
