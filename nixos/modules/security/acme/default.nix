{
    ...
    origBaseList: workingBaseList: needAssignmentList:
      ...
      then []
        ...
        ++ _rrCycler origBaseList (
          if (tail workingBaseList == [])
          then origBaseList
          else tail workingBaseList
  ...
  roundRobinApplyAttrs = funcsAttrs: argsList:
    lib.listToAttrs (
      map (x: {
        inherit (x.snd) name;
  ...
  certToConfig = cert: data: let
    ...
    protocolOpts =
      ...
      else if data.s3Bucket != null
      then [
        "--http"
        "--http.s3-bucket"
        data.s3Bucket
      ]
      else if data.listenHTTP != null
      then [
        "--http"
        "--http.port"
        data.listenHTTP
    ...
    selfsignService = lockfileName: {
      ...
      script =
        (
          ...
          then lib.id
          else wrapInFlock "${lockdir}${lockfileName}"
        ) ''
          minica \
            --ca-key ca/key.pem \
            --ca-cert ca/cert.pem \
  ...
  inheritableModule = isDefaults: {config, ...}: let
    defaultAndText = name: default: {
      ...
      defaultText =
        if isDefaults
        then default
        else lib.literalExpression "config.security.acme.defaults.${name}";
    };
  in {
    imports = [
    ...
    options = {
      ...
      enableDebugLogs =
        lib.mkEnableOption "debug logging for this certificate"
        // {
      ...
      server = lib.mkOption {
        type = lib.types.nullOr lib.types.str;
        inherit
          (defaultAndText "server" "https://acme-v02.api.letsencrypt.org/directory")
          default
          defaultText
          ;
      ...
      systemd.targets = let
        ...
        accountTargets = lib.mapAttrs' (
          hash: confs: let
            ...
            leaderConf =
              if dnsConfs != []
              then builtins.head dnsConfs
              else builtins.head confs;
