...
with lib; let
  ...
  in {
    ...
    path =
      ...
      else if cfg.backend == "podman"
      then [config.virtualisation.podman.package]
    ...
    preStop =
      ...
      then "podman stop --ignore --cidfile=/run/${escapedName}/ctr-id"
      else "${cfg.backend} stop ${name} || true";

    postStop =
      if cfg.backend == "podman"
      then "podman rm -f --ignore --cidfile=/run/${escapedName}/ctr-id"
...
in {
  ...
  options.virtualisation.oci-containers = {
    backend = mkOption {
      ...
      default =
        if versionAtLeast config.system.stateVersion "22.05"
        then "podman"
        else "docker";
