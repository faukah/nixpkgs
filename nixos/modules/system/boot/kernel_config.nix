...
with lib; let
  mergeFalseByDefault = locs: defs:
    ...
    else if any (x: x == false) (getValues defs)
    then false
    else true;
  ...
  mkValue = with lib;
    ...
    in
      if (val == "")
      then "\"\""
      else if val == "y" || val == "m" || val == "n"
      then val
      else if all isNumber (stringToCharacters val)
      then val
      else if substring 0 2 val == "0x"
      then val
  ...
  generateNixKConf = exprs: let
    mkConfigLine = key: item: let
      val =
        if item.freeform != null
        then item.freeform
        else item.tristate;
    in
