{
  ...
  options = {
    systemd.tpm2.enable =
      lib.mkEnableOption "systemd TPM2 support"
      // {
    ...
    boot.initrd.systemd.tpm2.enable =
      lib.mkEnableOption "systemd initrd TPM2 support"
      // {
      ...
      in
        lib.mkIf (cfg.enable && cfg.tpm2.enable) {
          ...
          boot.initrd.availableKernelModules =
            ["tpm-tis"]
            ++ lib.optional (!(pkgs.stdenv.hostPlatform.isRiscV64 || pkgs.stdenv.hostPlatform.isArmv7)) "tpm-crb";
