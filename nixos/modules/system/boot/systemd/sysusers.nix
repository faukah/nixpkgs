{
  ...
  sysusersConfig = pkgs.writeTextDir "00-nixos.conf" ''
    ...
    ${lib.concatLines (
      lib.mapAttrsToList (
        username: opts: let
          uid =
            if opts.uid == null
            then "/var/lib/nixos/uid/${username}"
            else toString opts.uid;
        in ''u ${username} ${uid}:${opts.group} "${opts.description}" ${opts.home} ${utils.toShellPath opts.shell}''
      )
      systemUsers
    )}
    ...
    ${lib.concatLines (
      lib.mapAttrsToList (
        groupname: opts: ''g ${groupname} ${
            if opts.gid == null
            then "/var/lib/nixos/gid/${groupname}"
            else toString opts.gid
          }''
      )
      userCfg.groups
    )}
  ...
  passwordFilesLocation =
    if immutableEtc
    then immutablePasswordFilesLocation
    else "/etc";
...
in {
  options = {
    ...
    systemd.sysusers = {
      enable =
        lib.mkEnableOption "systemd-sysusers"
        // {
          description = ''
