...
in let
  ...
  prepareImage = ''
    ...
    ${
      ...
      then ''
        ${
          if partitionTableType == "efi" || partitionTableType == "efixbootldr" || partitionTableType == "hybrid"
          then ''
          ...
          ''
          else if partitionTableType == "legacy+boot"
          then ''
...
in
  if onlyNixStore
  then pkgs.runCommand name {} (prepareImage + moveOrConvertImage + createHydraBuildProducts + postVM)
  else buildImage
