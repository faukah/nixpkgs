...
in let
  utils = rec {
    ...
    toShellPath = shell:
      ...
      else if types.package.check shell
    ...
    */
    recursiveGetAttrsetWithJqPrefix = item: attr: let
      recurse = prefix: item:
        if item ? ${attr}
        then nameValuePair prefix item
        else if isDerivation item
        then []
        ...
        then imap0 (index: item: recurse (prefix + "[${toString index}]") item) item
        else [];
    in
    ...
    */
    ...
    genJqSecretsReplacementSnippet' = attr: set: output: let
    ...
    in
      ...
      + escapeShellArg (
        stringOrDefault (concatStringsSep " | " (
          imap1 (
            index: name: ''${name} = ($ENV.secret${toString index}${optionalString (!secrets.${name}.quote) " | fromjson"})''
          ) (attrNames secrets)
        )) "."
      )
      + ''
    ...
    */
    ...
    Example:
      ...
      => true
    */
    disablePackageByName = package: packagesToDisable: let
