{lib}: let
  ...
  */
        ...
        typeSet =
          ...
          then
            ...
            then
              ...
              then {
                type =
                  if mergedType ? functor.wrappedDeprecationMessage
                  then addDeprecatedWrapped mergedType
                  else mergedType;
              }
              else
            ...
            else if opt.options.type ? functor.wrappedDeprecationMessage
            then {type = addDeprecatedWrapped opt.options.type;}
            else {}
          else {};
      ...
      in
        ...
        then
          ...
          submodules =
            ...
            then map (setDefaultModuleLocation opt._file) getSubModules ++ res.options
            else res.options;
        in
          opt.options
          // res
  ...
  : 3\. Function argument
  */
  evalOptionValue = loc: opt: defs: let
    ...
    res =
      if opt.readOnly or false && length defs' > 1
      then let
    ...
    value =
      if opt ? apply
      then opt.apply res.mergedValue
      else res.mergedValue;
  ...
  mergeDefinitions = loc: type: defs: rec {
    ...
    mergedValue =
      if isDefined
      then
        if all (def: type.check def.value) defsFinal
  ...
  */
  dischargeProperties = def:
    if def._type or "" == "merge"
    then concatMap dischargeProperties def.contents
    else if def._type or "" == "if"
    then
      if isBool def.condition
      then
        if def.condition
        then dischargeProperties def.content
        else []
      else throw "‘mkIf’ called with a non-Boolean condition"
    else [def];
  ...
  */
  ...
  ```
  */
  mergeAttrDefinitionsWithPrio = opt: let
    defsByAttr = zipAttrs (
      concatLists (
        concatMap (
          {value, ...} @ def:
  ...
  */
  ...
  mkAliasAndWrapDefsWithPriority = wrap: option: let
  ...
  */
  importApply = modulePath: staticArg: lib.setDefaultModuleLocation modulePath (import modulePath staticArg);
