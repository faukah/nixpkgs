...
let
  ...
  */
  ...
  _commitIdFromGitRepoOrError = let
    readCommitFromFile = file: path: let
      fileName = path + "/${file}";
  ...
  urlToName = url: let
    ...
    in
      if matchExt != null
      then lib.head matchExt
      else name;
    # apply function f to string x while the result shrinks
    shrink = f: x: let
      v = f x;
    in
      if stringLength v < stringLength x
      then shrink f v
      else x;
  in
  ...
  in
    if matchHash != null
    then builtins.substring 0 7 baseRev
    else if matchVer != null
    then lib.last matchVer
    else baseRev;
  ...
  repoRevToNameFull = repo_: rev_: suffix_: let
  ...
  # match on `kind` first to minimize the thunk
    ...
    then repoRevToNameFull
    else throw "repoRevToName: invalid kind";
in {
  pathType =
