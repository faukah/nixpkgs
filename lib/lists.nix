...
in rec {
  ...
  */
  ...
  :::
  */
  findSingle = pred: default: multiple: list: let
  ...
  */
  ...
  :::
  */
  listDfs = stopOnCycles: before: list: let
    dfs' = us: visited: rest: let
  ...
  naturalSort = lst: let
    vectorise = s:
      map (x:
        if isList x
        then toInt (head x)
        else x) (builtins.split "(0|[1-9][0-9]*)" s);
    prepared =
      map (x: [
        (vectorise x)
        x
  ...
  */
  ...
  :::
  */
  removePrefix = list1: list2:
    if hasPrefix list1 list2
    then drop (length list1) list2
    else throw "lib.lists.removePrefix: First argument is not a list prefix of the second argument";
