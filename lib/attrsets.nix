...
in rec {
  ...
  **Laws**:
  ...
  :::
  */
  longestValidPathPrefix = attrPath: v: let
    lenAttrPath = length attrPath;
    getPrefixForSetAtIndex =
      ...
      remainingPathIndex:
        if remainingPathIndex == lenAttrPath
        then
  ...
  */
  updateManyAttrsByPath = let
    ...
    # entries.
    go = prefixLength: hasValue: value: updates: let
        ...
        # Missing attr, not equal.
          ...
          && (
            ...
            in
              ...
              if isAttrs lhs
              then isAttrs rhs && matchAttrs lhs rhs
              else lhs == rhs
          )
      ) (attrNames pattern);
