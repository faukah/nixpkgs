...
in rec {
    ...
    args:
      ...
      then args
        ...
        h =
          ...
          in
            if hashesAsNVPairs == []
            then throwIf required "fetcher called without `hash`" null
            else if tail hashesAsNVPairs != []
            then
              throw "fetcher called with mutually-incompatible arguments: ${
                concatMapStringsSep ", " (a: a.name) hashesAsNVPairs
              }"
            else head hashesAsNVPairs;
      in
        ...
        // (optionalAttrs (h != null) {
          ...
          outputHash =
            if h.value == ""
            then fakeH.${h.name} or (throw "no “fake hash” defined for ${h.name}")
            else h.value;
        });
  ...
  */
  withNormalizedHash = {hashTypes ? ["sha256"]}: fetcher: let
