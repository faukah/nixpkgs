...
in
  rec {
    ...
    */
    ...
    : 4\. Function argument
    */
    mkKeyValueDefault = {mkValueString ? mkValueStringDefault {}}: sep: k: v: "${escape [sep] k}${sep}${mkValueString v}";
    ...
    */
    withRecursion = {
      ...
      assert isInt depthLimit; let
        ...
        in
          if isAttrs v
          then mapAttrs (stepIntoAttr evalNext) v
          else if isList v
          then map evalNext v
    ...
    toPretty = {
      ...
      in
        ...
        then
          if v == []
          then "[ ]"
          else "[" + introSpace + concatMapStringsSep introSpace (go (indent + "  ")) v + outroSpace + "]"
        else if isFunction v
        then let
          fna = functionArgs v;
          showFnas = concatStringsSep ", " (
            mapAttrsToList (name: hasDefVal:
              if hasDefVal
              then name + "?"
              else name)
            fna
        ...
        then
          ...
          then "<derivation ${v.name or "???"}>"
            ...
            + concatStringsSep introSpace (
              mapAttrsToList (
                name: value: "${escapeNixIdentifier name} = ${
    ...
    */
    ...
    in
      if isAttrs v
      then "{ ${concatItems (mapAttrsToList (key: value: "${key} = ${toDhall args value}") v)} }"
      else if isList v
      then "[ ${concatItems (map (toDhall args) v)} ]"
      else if isInt v
      then "${
        if v < 0
        then ""
      ...
      then abort "generators.toDhall: cannot convert a function to Dhall"
      else if v == null
      then abort "generators.toDhall: cannot convert a null to Dhall"
      else toJSON v;
