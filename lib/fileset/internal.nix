...
in
  ...
  rec {
    ...
    _coerce = context: value:
      if value._type or "" == "fileset"
      then
        if value._internalVersion > _currentVersion
        then
          throw ''
      ...
      then
        if value ? _isLibCleanSourceWith
        then
          throw ''
        ...
        else throw ''${context} is of type ${typeOf value}, but it should be a file set or a path instead.''
      else if !pathExists value
      then
        throw ''
          ${context} (${toString value}) is a path that does not exist.
              To create a file set from a path that may not exist, use `lib.fileset.maybeMissing`.''
      else _singleton value;
    ...
    _coerceMany = functionContext: list: let
      filesets = map ({
        context,
        value,
      }:
    ...
    in
      ...
      if firstWithBase != null && differentIndex != null
      then
        throw ''
    ...
    */
    _normaliseTreeFilter = path: tree:
      if tree == "directory" || isAttrs tree
      then let
    ...
    */
    _normaliseTreeMinimal = path: tree:
      ...
      in
        ...
        if all (value: value == "emptyDir") subtreeValues
        then "emptyDir"
        ...
        else if tree == "directory" || all (value: isString value) subtreeValues
        then "directory"
        ...
        else if all (value: isNull value || value == "emptyDir") subtreeValues
        then null
    ...
    _printMinimalTree = base: tree: let
      treeSuffix = tree:
        if isAttrs tree
        then ""
        else if tree == "directory"
        then " (all files in directory)"
        else
      ...
      traceTreeAttrs = prevLine: indent: tree:
        foldl' (
          ...
          in
            if subtree == null || subtree == "emptyDir"
            then
              # Don't print anything at all if this subtree is empty
              prevLine
            else if isAttrs subtree
            then
      ...
      firstLine =
        if tree == null || tree == "emptyDir"
        then trace "(empty)" null
        else trace "${toString base}${treeSuffix tree}" null;
    in
      if isAttrs tree
      then traceTreeAttrs firstLine "" tree
      else firstLine;

    # Pretty-print a file set in a pretty way when the resulting value is evaluated
    # Type: fileset -> Null
    _printFileset = fileset:
      if fileset._internalIsEmptyWithoutBase
      then trace "(empty)" null
      else
    ...
    _toSourceFilter = fileset: let
      ...
      baseString =
        if fileset._internalBaseComponents == []
        then
          # Need to handle the filesystem root specially
          "/"
        else "/" + concatStringsSep "/" fileset._internalBaseComponents + "/";
      ...
      # Type: [ String ] -> Bool
      inTree = components: let
        recurse = index: localTree:
          if isAttrs localTree
          then
    ...
    in
      ...
      if fileset._internalIsEmptyWithoutBase || tree == null
      then empty
      else nonEmpty;
    ...
    _fromSourceFilter = root: sourceFilter: let
      ...
      # The filter needs to run on the path as a string
        if !sourceFilter pathString type
        then null
        else if type == "directory"
        then fromDir path pathString
        else type;
    ...
    _toList = fileset: let
      recurse = path: tree:
        if isAttrs tree
        then concatLists (mapAttrsToList (name: value: recurse (path + "/${name}") value) tree)
        else if tree == "directory"
        then recurse path (readDir path)
        else if tree == null
        then []
        else [path];
    in
      if fileset._internalIsEmptyWithoutBase
      then []
      else recurse fileset._internalBase fileset._internalTree;

    # Transforms the filesetTree of a file set to a shorter base path, e.g.
    # _shortenTreeBase [ "foo" ] (_create /foo/bar null)
    # => { bar = null; }
    _shortenTreeBase = targetBaseComponents: fileset: let
      recurse = index:
      # If we haven't reached the required depth yet
        if index < length fileset._internalBaseComponents
        then
    ...
    # => { baz = "regular"; }
    _lengthenTreeBase = targetBaseComponents: fileset: let
      recurse = index: tree:
      # If the filesetTree is an attribute set and we haven't reached the required depth yet
        if isAttrs tree && index < length targetBaseComponents
        then
    ...
    in
      ...
      if filesetsWithBase == []
      then _emptyWithoutBase
      else _create commonBase resultTree;
    ...
    _intersection = fileset1: fileset2: let
      ...
      longestBaseFileset =
        if commonBaseComponentsLength == length fileset1._internalBaseComponents
        then
    ...
    in
      ...
      then _emptyWithoutBase
      else _create longestBaseFileset._internalBase resultTree;
    ...
    # The filesets must already be coerced and validated to be in the same filesystem root.
    # Type: Fileset -> Fileset -> Fileset
    _difference = positive: negative: let
      # The common base components prefix, e.g.
      # (/foo/bar, /foo/bar/baz) -> /foo/bar
      # (/foo/bar, /foo/baz) -> /foo
      commonBaseComponentsLength =
      ...
      negativeTreeWithPositiveBase =
        if commonBaseComponentsLength == length positive._internalBaseComponents
        then
        ...
        else if commonBaseComponentsLength == length negative._internalBaseComponents
        then
    ...
    _fileFilter = predicate: root: let
      ...
      fromDir = path:
        mapAttrs (
          name: type:
            if type == "directory"
            then fromDir (path + "/${name}")
            else fromFile name type
        ) (readDir path);

      rootType = pathType root;
    in
      if rootType == "directory"
      then _create root (fromDir root)
      else
    ...
    _mirrorStorePath = localPath: storePath: let
      recurse = focusedStorePath:
        mapAttrs (
          name: type:
            if type == "directory"
            then recurse (focusedStorePath + "/${name}")
            else type
    ...
    _fromFetchGit = function: argument: path: extraFetchGitAttrs: let
      ...
      in
        ...
        if !pathExists (path + "/.git")
        then throw "lib.fileset.${function}: Expected the ${argument} (${toString path}) to point to a local working tree of a Git repository, but it's not."
        else _mirrorStorePath path fetchResult.outPath;
    in
      if !isPath path
      then throw "lib.fileset.${function}: Expected the ${argument} to be a path, but it's a ${typeOf path} instead."
      else if pathType path != "directory"
      then throw "lib.fileset.${function}: Expected the ${argument} (${toString path}) to be a directory, but it's a file instead."
      else if hasStorePathPrefix path
      then tryStorePath
      else tryFetchGit;
  }
