...
in {
  ...
  config = {
    ...
    okChecks = builtins.addErrorContext "while evaluating the assertions" (
      ...
      assert config.docs."submodules.<name>.qux".loc
      == [
        "submodules"
        "<name>"
        "qux"
      ...
      assert options.merged.declarations
      == [
        __curPos.file
        __curPos.file
      ];
      ...
      assert (lib.elemAt options.merged.declarationPositions 0).line
      != (lib.elemAt options.merged.declarationPositions 1).line;
      assert mergedSubOption.declarations
      == [
        __curPos.file
        __curPos.file
      ];
      ...
      assert (lib.elemAt mergedSubOption.declarationPositions 0).line
      != (lib.elemAt mergedSubOption.declarationPositions 1).line;
      assert lib.length config.docs."merged.<name>.extensible".declarations == 2; true
    );
  };
}
