...
{lib}: let
  ...
  inAttrPosSuffix = v: name: let
    pos = builtins.unsafeGetAttrPos name v;
  in
    if pos == null
    then ""
    else " at ${pos.file}:${toString pos.line}:${toString pos.column}";
  ...
  makeWrappedDeprecationMessage = payload: {loc}:
    lib.warn ''
  ...
  outer_types = rec {
    ...
    defaultTypeMerge = f: f': let
      mergedWrapped = f.wrapped.typeMerge f'.wrapped.functor;
      mergedPayload = f.binOp f.payload f'.payload;

      hasPayload = assert (f'.payload != null) == (f.payload != null);
        f.payload != null;
      hasWrapped = assert (f'.wrapped != null) == (f.wrapped != null);
        f.wrapped != null;

      typeFromPayload =
        if mergedPayload == null
        then null
        else f.type mergedPayload;
      typeFromWrapped =
        if mergedWrapped == null
        then null
        else f.type mergedWrapped;
    in
      ...
      then
        ...
        if f ? wrappedDeprecationMessage
        then typeFromPayload
        else if hasWrapped
        then
          ...
          ''
        else typeFromPayload
      else if hasWrapped
      then typeFromWrapped
      else f.type;
    ...
    mkOptionType = {
      ...
      inherit
        ...
        ;
      functor =
        if functor ? wrappedDeprecationMessage
        then
          functor
          // {
    ...
    types = rec {
      ...
      package = mkOptionType {
        ...
        in
          if builtins.isPath res || (builtins.isString res && !builtins.hasContext res)
        ...
        mkOptionType
        {
          ...
          check = x: let
            isInStore = lib.path.hasStorePathPrefix (
              ...
              else /. + builtins.unsafeDiscardStringContext x
            );
            isAbsolute = builtins.substring 0 1 (toString x) == "/";
            isExpectedType = (
              if inStore == null || inStore
              then isStringLike x
              else isString x # Do not allow a true path, which could be copied to the store later on.
            );
          in
            isExpectedType
      ...
      listOf = elemType:
        mkOptionType rec {
          ...
          functor =
            (elemTypeFunctor name {inherit elemType;})
            // {
      ...
      attrsWith = let
        ...
        binOp = lhs: rhs: let
          ...
          placeholder =
            ...
            then lhs.placeholder
            else if lhs.placeholder == "name"
            then rhs.placeholder
            else if rhs.placeholder == "name"
            then lhs.placeholder
            else null;
        in
          if elemType == null || lazy == null || placeholder == null
          then null
          else {
      ...
      unique = {message}: type:
        mkOptionType rec {
          ...
          functor =
            elemTypeFunctor name {elemType = type;}
            // {
      ...
      nullOr = elemType:
        mkOptionType rec {
          ...
          in
            if nrNulls == length defs
            then null
            else if nrNulls != 0
            then throw "The option `${showOption loc}` is defined both null and not null, in ${showFiles (getFiles defs)}."
      ...
      functionTo = elemType:
        mkOptionType {
          ...
          functor =
            (elemTypeFunctor "functionTo" {inherit elemType;})
            // {
      ...
      in
        mkOptionType rec {
          ...
          description =
            ...
            # identifier.
            if values == []
            then "impossible (empty enum)"
            else if builtins.length values == 1
            then "value ${show (builtins.head values)} (singular enum)"
            else "one of ${concatMapStringsSep ", " show values}";
          descriptionClass =
      ...
      either = t1: t2:
        mkOptionType rec {
          ...
          description =
            ...
            else "${optionDescriptionPhrase (class: class == "noun" || class == "conjunction") t1} or ${
    ...
    */
    mergeTypes = a: b:
      assert isOptionType a && isOptionType b; let
        merged = a.typeMerge b.functor;
      in
        if merged == null
        then setType "merge-error" {error = "Cannot merge types";}
        else merged;
  };
in
