...
in rec {
        ...
        ${lib.concatMapStringsSep "\n" (
            {
              ...
              type,
              ...
            }: "- `lib.gvariant.${name}` for `${type}`"
          )
          validConstructors}
      ''
    else if builtins.isAttrs v
    then throw "Cannot construct GVariant value from an attribute set. If you want to construct a dictionary, you will need to create an array containing items constructed with `lib.gvariant.mkDictionaryEntry`."
    else throw "The GVariant type of “${builtins.typeOf v}” can't be inferred.";
  ...
  */
  mkArray = elems: let
    ...
    elemType =
      lib.throwIfNot (lib.all (t: (head vs).type == t) (
        map (v: v.type) vs
  ...
  */
  ...
  :::
  */
  mkDictionaryEntry = name: value: let
    name' = mkValue name;
    value' = mkValue value;
  ...
  */
  mkMaybe = elemType: elem:
    ...
    // {
      __toString = self:
        if self.value == null
        then "@${self.type} nothing"
        else "just ${toString self.value}";
    };
