{lib}: let
  ...
  defaultMergeArg = x: y:
    if builtins.isAttrs y
    then y
    else (y x);
  defaultMerge = x: y: x // (defaultMergeArg x y);
  foldArgs = merger: f: init: x: let
  ...
  getValue = attrSet: argList: name: (attrByPath [name] (
      ...
      in
        if (head x) == name
        then (head (tail x))
        else (getValue attrSet (tail argList) name)
    )
    attrSet);

  # Input : attrSet, [[name default] ...], [ [flagname reqs..] ... ]
  # Output : are reqs satisfied? It's asserted.
  checkReqs = attrSet: argList: condList: (foldr and true (
    map (
      x: let
        name = head x;
      in (
  ...
  condConcat = name: list: checker:
    ...
    then condConcat (name + (head (tail list))) (tail (tail list)) checker
    else condConcat name (tail (tail list)) checker;
  ...
  in
    foldr mergeAttrs {} [
      ...
      (
        ...
        (
          a: v:
          # merge special names using given functions
            if x ? ${a}
            then
              if y ? ${a}
