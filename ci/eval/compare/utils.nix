{lib, ...}: rec {
  ...
  */
  convertToPackagePlatformAttr = packagePlatformPath: let
  ...
  */
  convertToPackagePlatformAttrs = packagePlatformPaths:
  ...
  */
  extractPackageNames = packagePlatformPaths: let
  ...
  */
  groupByPlatform = packagePlatformAttrs: let
  ...
  #   darwin = [ "hello" "bye" ];
  # }
  groupByKernel = packagePlatformAttrs: let
    filterKernel = kernel:
      builtins.attrNames (
  ...
  */
  getLabels = rebuildCountByKernel:
    lib.concatLists (
      lib.mapAttrsToList (
        kernel: rebuildCount: let
          numbers =
            if rebuildCount == 0
            then ["0"]
            else if rebuildCount == 1
            then [
              "1"
              "1-10"
            ]
            else if rebuildCount <= 10
            then ["1-10"]
            else if rebuildCount <= 100
            then ["11-100"]
            else if rebuildCount <= 500
            then ["101-500"]
            else if rebuildCount <= 1000
            then [
              "501-1000"
              "501+"
            ]
            else if rebuildCount <= 2500
            then [
              "1001-2500"
              "501+"
            ]
            else if rebuildCount <= 5000
            then [
              "2501-5000"
              "501+"
            ]
