...
in
  {
    url,
    name ?
      if (builtins.match urlRegexp url) == null
      then "bittorrent"
      else "bittorrent-" + builtins.head (builtins.match urlRegexp url),
    config ?
      if (backend == "transmission")
      then {}
      else throw "json config for configuring fetchFromBitorrent only works with the transmission backend",
    hash,
  ...
  in
    runCommand name
    {
      ...
      nativeBuildInputs =
        ...
        ++ (
          if (backend == "transmission")
          then [transmission_3_noSystemd]
          else if (backend == "rqbit")
          then [rqbit]
          else throw "rqbit or transmission are the only available backends for fetchtorrent"
        );
      outputHashAlgo =
        if hash != ""
        then null
