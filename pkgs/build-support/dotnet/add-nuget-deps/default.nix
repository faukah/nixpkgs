{
  ...
  */
  ...
  deps =
    if nugetDeps == null
    then []
    else if lib.isDerivation nugetDeps
    then [nugetDeps]
    else if lib.isList nugetDeps
    then nugetDeps
    else if lib.hasSuffix ".nix" nugetDeps
    then
      assert (lib.isPath nugetDeps);
        callPackage nugetDeps {fetchNuGet = fetchNupkg;}
    else builtins.map fetchNupkg (lib.importJSON nugetDeps);
      ...
      // lib.optionalAttrs (nugetDeps == null || lib.isPath nugetDeps) rec {
        ...
        fetch-deps = let
          ...
          defaultDepsFile =
            ...
            if lib.isPath nugetDeps && !lib.isStorePath nugetDeps
            then toString nugetDeps
            else ''$(mktemp -t "${finalAttrs.pname or finalPackage.name}-deps-XXXXXX.nix")'';
        in
