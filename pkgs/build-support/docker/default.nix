...
in rec {
  ...
  buildImage = lib.makeOverridable (
    args @ {
      ...
      extraCommandsWithDB =
        if includeNixDB
        then (mkDbExtraCommand rootContents) + extraCommands
        else extraCommands;
  ...
  streamNixShellImage = {
    ...
    ) "streamNixShellImage: Can't specify both command and run"; let
