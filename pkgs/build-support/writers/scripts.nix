...
in rec {
  ...
  */
  makeScriptWriter = {
    ...
    assert (types.path.check content) || (types.str.check content); let
  ...
  */
  writeGuile = nameOrPath: {
    ...
    assert builtins.all builtins.isInt srfi; let
  ...
  */
  writeHaskell = name: {
    ...
    ghcArgs' =
      if threadedRuntime
      then appendIfNotSet "-threaded" ghcArgs
      else ghcArgs;
  in
    makeBinWriter {
      compileScript = ''
  ...
  */
  ...
  :::
  */
  writeJS = name: {libraries ? []}: content: let
  ...
  */
  ...
  makeFSharpWriter = {
    ...
    path =
      if strings.hasSuffix ".fsx" nameOrPath
      then nameOrPath
      else "${nameOrPath}.fsx";
