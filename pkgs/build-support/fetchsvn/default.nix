{
  ...
  repoToName = url: rev: let
    ...
    repoName =
      ...
      else if elemAt path 1 == "branches"
      then "${elemAt path 2}-${head path}"
      # ../repo/tags/tag -> repo-tag
      else if elemAt path 1 == "tags"
      then "${elemAt path 2}-${head path}"
      # ../repo (no trunk) -> repo
      else head path;
...
in
  {
    ...
    assert sshSupport -> openssh != null;
      if hash != "" && sha256 != ""
      then throw "Only one of sha256 or hash can be set"
      else
        stdenvNoCC.mkDerivation {
          ...
          SVN_SSH =
            if sshSupport
            then "${buildPackages.openssh}/bin/ssh"
            else null;
