...
in rec {
  ...
  # string -> [[regex bool]]
  gitignoreToPatterns = gitignore: let
    # ignore -> bool
  ...
  withRecursiveGitignoreFile = patterns: root: toList patterns ++ [".git"] ++ [(compileRecursiveGitignore root)];
  ...
  in
    ...
    then throw "type error in gitignoreSource(patterns -> source -> path), " "use [] or \"\" if there are no additional patterns"
