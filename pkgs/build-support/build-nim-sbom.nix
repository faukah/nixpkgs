{
  ...
  filterPropertiesToAttrs = prefix: properties:
    lib.pipe properties [
  ...
  applyOverrides = prevAttrs:
    builtins.foldl' (
      prevAttrs: {name, ...} @ component:
  ...
  compose = callerArg: sbom: finalAttrs: let
    callerAttrs =
      if builtins.isAttrs callerArg
      then callerArg
      else callerArg finalAttrs;
    sbomAttrs = callerAttrs // (applySbom sbom callerAttrs);
    overrideAttrs = sbomAttrs // (applyOverrides sbomAttrs);
  in
    overrideAttrs;
in
  callerArg: sbomArg: let
    sbom =
      if builtins.isAttrs sbomArg
      then sbomArg
      else builtins.fromJSON (builtins.readFile sbomArg);
