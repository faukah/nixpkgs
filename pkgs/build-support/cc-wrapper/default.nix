...
# compiler and the linker just "work".
{
  ...
  else if (with stdenvNoCC; buildPlatform != hostPlatform || hostPlatform != targetPlatform)
  then false
  ...
  else if (cc.passthru.isFromBootstrapFiles or false)
  then false
  # Add these flags when wrapping `xgcc` (the first compiler that nixpkgs builds)
  else if (cc.passthru.isXgcc or false)
  then true
  # Add these flags when wrapping `stdenv.cc`
  else if (cc.stdenv.cc.cc.passthru.isXgcc or false)
  then true
...
assert (noLibc || nativeLibc) == (libc == null); let
  ...
  # for x86 -mtune= takes the same values as -march, plus two more:
    ...
    else if targetPlatform.isAarch64
    then
      (
        if isGNU
        then
    ...
    else if targetPlatform.isPower
    then
  ...
  findBestTuneApproximation = tune: let
    guess =
      if isClang
      then
        {
  ...
  in
    if isGccTuneSupported guess
    then guess
    else null;

  thumb =
    if targetPlatform.gcc.thumb
    then "thumb"
    else "arm";
  tune =
    if targetPlatform ? gcc.tune
  ...
  darwinPlatformForCC = optionalString targetPlatform.isDarwin (
    if (targetPlatform.darwinPlatform == "macos" && isGNU)
    then "macosx"
