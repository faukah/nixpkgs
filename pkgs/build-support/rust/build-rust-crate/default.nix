    ...
    lib.concatMapStringsSep " " (
      dep: let
        ...
        filename =
          ...
          else if (lib.attrByPath ["procMacro"] false dep)
          then "${dep.metadata}${stdenv.buildPlatform.extensions.library}"
      ...
      in
        stdenv.mkDerivation (
          rec {
            ...
            crateAuthors =
              if crate ? authors && lib.isList crate.authors
              then crate.authors
              else [];
            ...
            crateType =
              if lib.attrByPath ["procMacro"] false crate
              then ["proc-macro"]
              else if lib.attrByPath ["plugin"] false crate
              then ["dylib"]
              else (crate.type or ["lib"]);
            colors = lib.attrByPath ["colors"] "always" crate;
            extraLinkFlags = lib.concatStringsSep " " (crate.extraLinkFlags or []);
            edition = crate.edition or null;
            codegenUnits =
              if crate ? codegenUnits
              then crate.codegenUnits
              else defaultCodegenUnits;
            extraRustcOpts =
