...
assert (lockFile == null) != (lockFileContents == null); let
  ...
  fetchCrate = pkg: downloadUrl: let
    checksum =
  ...
  in
    ...
    then let
      ...
      '';
      tree =
        if gitShaOutputHash ? ${gitParts.sha}
        then
          fetchgit {
