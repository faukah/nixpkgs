    ...
    "buildRustPackage: `useFetchCargoVendor` is non‐optional and enabled by default as of 25.05";
      ...
      // {
        cargoDeps =
          ...
          then null
          else if cargoDeps != null
          then cargoDeps
          else if cargoLock != null
          then importCargoLock cargoLock
          else if args.cargoHash or null == null
          then throw "cargoHash, cargoVendorDir, cargoDeps, or cargoLock must be set"
          else
            fetchCargoVendor (
              {
                inherit
