...
in rec {
  ...
  */
  symlinkJoin = args_ @ {
    name ?
      ...
      ) "symlinkJoin requires either a `name` OR `pname` and `version`"; "${args_.pname}-${args_.version}",
    paths,
    stripPrefix ? "",
    preferLocalBuild ? true,
    allowSubstitutes ? false,
  ...
  */
  linkFarm = name: entries: let
    entries' =
      if (lib.isAttrs entries)
      then entries
      # We do this foldl to have last-wins semantics in case of repeated entries
      else if (lib.isList entries)
      then lib.foldl (a: b: a // {"${b.name}" = b.path;}) {} entries
  ...
  */
  ...
  requireFile = {
    ...
    assert (name != null) || (url != null); let
      ...
      hashAlgo =
        if hash != null
        then (builtins.head (lib.strings.splitString "-" hash))
        else if sha256 != null
        then "sha256"
        else "sha1";
      hashAlgo_ =
  ...
  */
  applyPatches = {
    ...
    assert lib.assertMsg (!args ? meta) "applyPatches will not merge 'meta', change it in 'src' instead";
    assert lib.assertMsg (!args ? passthru) "applyPatches will not merge 'passthru', change it in 'src' instead";
