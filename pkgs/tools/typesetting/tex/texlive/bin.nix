...
let
  ...
  assertFixedHash = name: src:
    if !useFixedHashes || src ? outputHash
    then src
    else throw "The TeX Live package '${src.pname}' must have a fixed hash before building '${name}'.";
...
in
  rec {
    ...
    dvisvgm = stdenv.mkDerivation rec {
      ...
      src = assert lib.assertMsg (version == texlive.pkgs.dvisvgm.version)
