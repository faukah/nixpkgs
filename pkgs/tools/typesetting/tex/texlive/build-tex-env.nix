...
lib.fix (
  self: {
    ...
    ensurePkgSets = ps:
      if !__fromCombineWrapper && builtins.any isOldPkgList ps
      then let
    ...
    pkgList = rec {
      ...
      all = let
        ...
        keySet = p: {
          ...
          tlDeps =
            if p ? tlDeps
            then ensurePkgSets p.tlDeps
            else (p.requiredTeXPackages or (_: []) tl);
        };
      in
        # texlive.combine: the wrapper already resolves all dependencies
        if __fromCombineWrapper
        then requiredTeXPackages null
        else
          builtins.catAttrs "p" (
      ...
      nonbin =
        if __fromCombineWrapper
        then builtins.filter (p: p.tlType != "bin" && p.tlType != "tlpkg") all # texlive.combine: legacy filter
        else
          (
            if __combine
            then # texlive.combine: emulate old input ordering to avoid rebuilds
              lib.concatMap (
                p:
      ...
      sortedFormatPkgs =
        if __formatsOf != null
        then [__formatsOf]
        else builtins.sort (a: b: a.pname < b.pname) formatPkgs;
      formats =
        map (
          p:
            self {
              requiredTeXPackages = ps:
                [
                  ps.scheme-infraonly
                  p
                ]
    ...
    name =
      if __combine
      then "texlive-${__extraName}-${bin.texliveYear}${__extraVersion}" # texlive.combine: old name name
      else
        "texlive-${bin.texliveYear}-"
        + (
          if __formatsOf != null
          then "${__formatsOf.pname}-fmt"
          else "env"
        );
    ...
    passthru = {
      ...
      __overrideTeXConfig = newArgs: let
        appliedArgs =
  ...
  in
    ...
    if __combine || __formatsOf != null
    then out
    else lib.addMetaAttrs {inherit (pkgList) outputsToInstall;} out
)
