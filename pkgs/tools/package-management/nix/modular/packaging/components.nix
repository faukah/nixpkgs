{
  ...
  */
  ...
  localSourceLayer = finalAttrs: prevAttrs: let
    workDirPath =
    ...
    sources = assert prevAttrs.fileset._type == "fileset";
      prevAttrs.fileset;
  ...
  makeFetchedSourceLayer = finalScope: finalAttrs: prevAttrs: let
    workDirPath =
      # Ideally we'd pick finalAttrs.workDir, but for now `mkDerivation` has
      # the requirement that everything except passthru and meta must be
      # serialized by mkDerivation, which doesn't work for this.
  ...
  in {
    ...
    in
      if n == 0
      then prevAttrs.version
      else prevAttrs.version + "+${toString n}";
  ...
  */
  ...
  whenAtLeast = v: thing:
    if lib.versionAtLeast version v
    then thing
    else null;
in
  # This becomes the pkgs.nixComponents attribute set
  {
    ...
    */
    ...
    nix-everything =
      callPackage ../packaging/everything.nix {}
      // {
