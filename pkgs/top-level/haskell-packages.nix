  ...
  in {
    ...
    ghc947 = callPackage ../development/compilers/ghc/9.4.7.nix {
      bootPkgs =
        ...
        if stdenv.buildPlatform.isAarch
        then
          # TODO(@sternenseemann): package bindist
          bb.packages.ghc902
        # No suitable bindists for powerpc64le
        else if stdenv.buildPlatform.isPower64 && stdenv.buildPlatform.isLittleEndian
        then bb.packages.ghc902
        else bb.packages.ghc8107Binary;
    ...
    ghc963 = callPackage ../development/compilers/ghc/9.6.3.nix {
      bootPkgs =
        ...
        if stdenv.buildPlatform.isAarch32
        then bb.packages.ghc928
        else if stdenv.buildPlatform.isPower64 && stdenv.buildPlatform.isLittleEndian
        then bb.packages.ghc928
        else bb.packages.ghc924Binary;
      inherit (buildPackages.python3Packages) sphinx;
      # Need to use apple's patched xattr until
      # https://github.com/xattr/xattr/issues/44 and
      # https://github.com/xattr/xattr/issues/55 are solved.
    ...
    ghc981 = callPackage ../development/compilers/ghc/9.8.1.nix {
      bootPkgs =
        ...
        if stdenv.buildPlatform.isAarch32
        then bb.packages.ghc963
        else if stdenv.buildPlatform.isPower64 && stdenv.buildPlatform.isLittleEndian
        then bb.packages.ghc963
        else bb.packages.ghc963Binary;
      inherit (buildPackages.python3Packages) sphinx;
      # Need to use apple's patched xattr until
      # https://github.com/xattr/xattr/issues/44 and
      # https://github.com/xattr/xattr/issues/55 are solved.
    ...
    ghc984 = callPackage ../development/compilers/ghc/9.8.4.nix {
      bootPkgs =
        if stdenv.buildPlatform.isAarch64 && stdenv.buildPlatform.isMusl
        then bb.packages.ghc984Binary
        else if stdenv.buildPlatform.isAarch32
        then
          # For GHC 9.6 no armv7l bindists are available.
          bb.packages.ghc963
        else if stdenv.buildPlatform.isPower64 && stdenv.buildPlatform.isLittleEndian
    ...
    ghcHEAD = callPackage ../development/compilers/ghc/head.nix {
      bootPkgs =
        ...
        if stdenv.buildPlatform.isAarch32
        then bb.packages.ghc984
        else if stdenv.buildPlatform.isPower64 && stdenv.buildPlatform.isLittleEndian
        then bb.packages.ghc984
        else bb.packages.ghc984Binary;
      inherit (buildPackages.python3Packages) sphinx;
      # Need to use apple's patched xattr until
      # https://github.com/xattr/xattr/issues/44 and
      # https://github.com/xattr/xattr/issues/55 are solved.
