...
lib: pkgs: actuallySplice: let
  spliceReal = {
    ...
    merge = name: {
      ...
      in
        ...
        if lib.isDerivation defaultValue
        then
          augmentedValue
          // spliceReal {
        ...
        else if lib.isAttrs defaultValue
        then
          spliceReal {
  ...
  pkgsForCall =
    if actuallySplice
    then splicedPackagesWithXorg
    else packagesWithXorg;
in {
  ...
  generateSplicesForMkScope = attrs: let
    split = X:
      [X]
      ++ (
  ...
  __splicedPackages =
    if actuallySplice
    then splicedPackages // {recurseForDerivations = false;}
    else pkgs;
}
