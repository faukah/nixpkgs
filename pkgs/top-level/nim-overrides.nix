...
{
  jester = lockAttrs: {buildInputs ? [], ...}: {
    buildInputs = buildInputs ++ [openssl];
  };

  hts = lockAttrs: {buildInputs ? [], ...}: {
  ...
  nimraylib_now = lockAttrs: {buildInputs ? [], ...}: {
    buildInputs = buildInputs ++ [raylib];
  };

  sass = lockAttrs: {buildInputs ? [], ...}: {
    buildInputs = buildInputs ++ [libsass];
  };

  sdl2 = lockAttrs: {buildInputs ? [], ...}: {
