...
with self; let
  ...
  removeDistribute = alias:
    if lib.isDerivation alias
    then lib.dontDistribute alias
    else alias;
...
in
  mapAliases {
    ...
    fuse2fs =
      ...
      then e2fsprogs.fuse2fs
      else null; # Added 2022-03-27 preserve, reason: convenience, arch has a package named fuse2fs too.
    ...
    transmission-gtk =
      lib.warnOnInstantiate (transmission3Warning {
        suffix = "-gtk";
      })
