...
with pkgs; {
  ...
  inherit
    (callPackages ../tools/networking/ivpn/default.nix {})
    ivpn
    ivpn-service
    ;
  ...
  inherit
    (callPackages ../tools/security/bitwarden-directory-connector {})
  ...
  inherit
    (callPackage ../development/tools/godot {})
    godot3
  ...
  inherit
    (callPackage ../development/tools/lerna {})
    lerna_6
    lerna_8
    ;
  lerna = lerna_8;
  ...
  inherit
    ({
      limesuite =
        callPackage ../applications/radio/limesuite {
        };
  ...
  inherit
    (import ../servers/sql/percona-server pkgs)
    percona-server_8_0
    percona-server_8_4
    percona-server
    ;
  inherit
    (import ../tools/backup/percona-xtrabackup pkgs)
  ...
  inherit
    (callPackage ../development/compilers/crystal {})
    crystal_1_11
    crystal_1_14
    crystal_1_15
    crystal
  ...
  ghc =
    ...
    (
      ...
      then haskell.compiler.native-bignum.ghc98
      # ICEs horribly on i686, see https://gitlab.haskell.org/ghc/ghc/-/issues/25904
      # FIXME: remove when fixed
      else if stdenv.hostPlatform.isi686
      then haskell.compiler.ghc96
      ...
      lib.splitString "," (
        lib.removePrefix "--target=" (
          lib.elemAt (lib.filter (
            ...
            pkgsBuildBuild.rustc.unwrapped.configureFlags)
          0
        )
      )
    );
  ...
  electron_34 =
    if lib.meta.availableOn stdenv.hostPlatform electron-source.electron_34
    then electron-source.electron_34
    else electron_34-bin;
  electron_35 =
    if lib.meta.availableOn stdenv.hostPlatform electron-source.electron_35
    then electron-source.electron_35
    else electron_35-bin;
  electron_36 =
    if lib.meta.availableOn stdenv.hostPlatform electron-source.electron_36
  ...
  inherit
    (callPackages ../development/libraries/botan {
      ...
      stdenv =
        if stdenv.hostPlatform.isStatic
        then buildPackages.libcxxStdenv
        else stdenv;
    })
    botan2
    botan3
  ...
  in
    ...
    then windows.mingw_w64_headers or fallback
    else if libc == "nblibc"
    then netbsd.headers
    else null;
  ...
  in
    ...
    then uclibc
    else if libc == "avrlibc"
    then avrlibc
    else if libc == "newlib" && stdenv.hostPlatform.isMsp430
    then msp430Newlib
    else if libc == "newlib" && stdenv.hostPlatform.isVc4
    then vc4-newlib
    else if libc == "newlib" && stdenv.hostPlatform.isOr1k
    then or1k-newlib
    else if libc == "newlib"
    then newlib
    else if libc == "newlib-nano"
    then newlib-nano
    else if libc == "musl"
    then musl
    else if libc == "msvcrt"
    ...
    then windows.mingw_w64
    else if libc == "libSystem"
    then
      if stdenv.hostPlatform.useiOSPrebuilt
      then darwin.iosSdkPkgs.libraries
    ...
    then netbsd.libc
    else if libc == "wasilibc"
    then wasilibc
    else if libc == "relibc"
    then relibc
    else if name == "llvm"
    then llvmPackages_20.libc
    else throw "Unknown libc ${libc}";
  ...
  mesa =
    if stdenv.hostPlatform.isDarwin
    then callPackage ../development/libraries/mesa/darwin.nix {}
  ...
  inherit
    (callPackages ../servers/http/jetty {})
    jetty_11
    jetty_12
    ;
  ...
  inherit
    (import ../servers/sql/postgresql pkgs)
    postgresqlVersions
  ...
  inherit
    (callPackages ../data/fonts/open-relay {})
    constructium
    fairfax
    fairfax-hd
    kreative-square
  ...
  inherit
    (callPackage ../applications/networking/cluster/rke2 {})
    rke2_1_30
    rke2_1_31
    rke2_1_32
    rke2_stable
