  ...
  in
    lib.makeOverridable (
      attrs: let
        ...
        stdenv' =
          if attrsFixed.noCC or false
          then stdenvNoCC
          else if attrsFixed.noBootstrap or false
          then stdenv
          else bootstrapStdenv;
      in
        stdenv'.mkDerivation (
          lib.extends (
            self: super:
              assert super ? releaseName; let
                  ...
                  // lib.optionalAttrs (super ? xcodeHash) {
                    postUnpack =
                      ...
                      + lib.concatMapStrings (
                        file:
                          if baseNameOf file == "meson.build.in"
                          then "substitute ${lib.escapeShellArg "${file}"} \"$sourceRoot/meson.build\" --subst-var version\n"
