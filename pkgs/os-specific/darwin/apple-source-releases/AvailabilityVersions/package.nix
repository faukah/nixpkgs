...
in
  mkAppleDerivation (finalAttrs: {
    ...
    installPhase = ''
      ...
      ${
        if lib.versionOlder (lib.getVersion xnu) "6153.11.26"
        then ''--replace-fail "\''${SDKROOT}/usr/local/libexec/availability.pl" "$out/libexec/availability" \''
        else ''--replace-fail "\''${SDKROOT}/\''${DRIVERKITROOT}/usr/local/libexec/availability.pl" "$out/libexec/availability" \''
      }
