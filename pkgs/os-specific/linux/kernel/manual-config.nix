  ...
  lib.makeOverridable (
    {
      ...
      modDirVersion_ =
        if modDirVersion == null
        then lib.versions.pad 3 version
        else modDirVersion;
    in let
      ...
      drvAttrs = config_: kernelConf: kernelPatches: configfile: let
        ...
        in
          {
            ...
            getValue = attr:
              if config.isSet attr
              then getAttr (attrName attr) config
              else null;
          ...
          installTargets = [
            (
              kernelConf.installTarget or (
                if kernelConf.target == "uImage" && stdenv.hostPlatform.linuxArch == "arm"
                then "uinstall"
                else if
                  (
                    kernelConf.target
          ...
          meta =
            {
              ...
              description =
                ...
                + (
                  if kernelPatches == []
                  then ""
                  else " (with patches: " + lib.concatStringsSep ", " (map (x: x.name) kernelPatches) + ")"
                );
