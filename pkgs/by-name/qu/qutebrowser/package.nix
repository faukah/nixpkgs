...
in
  python3.pkgs.buildPythonApplication {
    ...
    preFixup = let
      ...
      resourcesPath =
        if (isQt6 && stdenv.hostPlatform.isDarwin)
        then "${qt6Packages.qtwebengine}/lib/QtWebEngineCore.framework/Resources"
        else "${qt6Packages.qtwebengine}/resources";
    in ''
      makeWrapperArgs+=(
    ...
    meta = {
      ...
      platforms =
        if enableWideVine
        then ["x86_64-linux"]
        else qt6Packages.qtwebengine.meta.platforms;
