# Derivation containing the Limine host tool and the compiled bootloader
{
  ...
  hasX86 =
    (
      if targets == []
      then stdenv.hostPlatform.isx86_32
      else (builtins.elem "i686" targets)
    )
    || (
      if targets == []
      then stdenv.hostPlatform.isx86_64
      else (builtins.elem "x86_64" targets)
    )
    || enableAll;
  ...
  uefiFlags = target:
    ...
    .${
      target
    } or (throw "Unsupported target ${target}");
in
  ...
  stdenv.mkDerivation (finalAttrs: {
    ...
    configureFlags =
      ...
      ++ lib.concatMap uefiFlags (
        if targets == []
        then [stdenv.hostPlatform.parsed.cpu.name]
        else targets
      )
      ++ [
