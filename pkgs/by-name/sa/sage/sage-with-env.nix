...
# executable sage. No tests are run yet and no documentation is built.
  let
    ...
    pkg_to_spkg_name = pkg: patch_names: let
      ...
      # remove python3.8-
      stripped_parts =
        if (builtins.head parts) == python3.libPrefix
        then builtins.tail parts
        else parts;
    ...
    transitiveClosure = dep:
      ...
      then
        ...
        [dep]
        ++ (
          if builtins.hasAttr "propagatedBuildInputs" dep
          then lib.unique (builtins.concatLists (map transitiveClosure dep.propagatedBuildInputs))
          else []
        );
