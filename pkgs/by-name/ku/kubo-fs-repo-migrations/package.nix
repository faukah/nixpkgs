{
  ...
  mkMigration = from: to: version: hash: let
  ...
  in
    buildGoModule {
      ...
      checkPhase = ''
        ...
        ${
          if to <= 11
          then "" # Migrations fs-repo-10-to-11 and earlier require too much effort to test, making it not worth it
          else if to == 12
          then ''
            cd migration
            go test -mod=vendor
      ...
      meta = {
        ...
        broken =
          to
          == 7
          && stdenv.hostPlatform.isDarwin # fs-repo-6-to-7 is broken on macOS: gx/ipfs/QmSGRM5Udmy1jsFBr1Cawez7Lt7LZ3ZKA23GGVEsiEW6F3/eventfd/eventfd.go:27:32: undefined: syscall.SYS_EVENTFD2
          || (lib.elem to [
  ...
  stubBecauseDisabled = from: to: release: let
    pname = "fs-repo-${toString from}-to-${toString to}";
  in
    writeShellApplication {
      name = pname;
  ...
  minRepoVersionValidated =
    if minRepoVersion >= 0
    then minRepoVersion
    else throw "The minimum supported repo version is 0. Set `minRepoVersion` to a non-zero value.";

  latestMigration =
    builtins.foldl' (x: y:
      if y.to == maxRepoVersion
      then y
      else x) {
  ...
  mkMigrationOrStub = x: let
    builder =
      if x.from >= minRepoVersionValidated
      then mkMigration
      else stubBecauseDisabled;
  in
  ...
  migrationsBrokenStubbed =
    builtins.map (
      x:
        if packageNotBroken x
        then x
        else (stubBecauseBroken x.pname)
    )
    migrations;
in
  symlinkJoin {
    ...
    paths =
      if stubBrokenMigrations
      then migrationsBrokenStubbed
      else migrationsBrokenRemoved;
    meta =
      (builtins.removeAttrs kubo-migrator-unwrapped.meta ["mainProgram"])
      // {
