...
assert withSuperLuDist -> mpiSupport; let
  ...
  stdenv.mkDerivation (finalAttrs: {
    ...
    configureFlags =
      ...
      ++ lib.optionals (!mpiSupport) [
        "--with-cc=${stdenv.cc}/bin/${
          if stdenv.cc.isGNU
          then "gcc"
          else "clang"
        }"
        "--with-cxx=${stdenv.cc}/bin/${
          if stdenv.cc.isGNU
          then "g++"
          else "clang++"
        }"
    ...
    postInstall = lib.concatStringsSep "\n" (
      map (
        package: let
          ...
          prefix =
            if (pname == "blas" || pname == "lapack")
            then "BLASLAPACK"
            else lib.toUpper (builtins.elemAt (lib.splitString "-" pname) 0);
        in ''
