...
in
  {
    ...
    moduleArgsOverridingCompat = argName:
      if args.${argName} or null == null
      then _defaultGoVendorArgs.${argName}
      else
        lib.warn
  ...
  in
    (buildGoModule {
      ...
      configureFlags =
        [
          "--localstatedir=${
            if externalLocalStateDir != null
            then externalLocalStateDir
            else "${placeholder "out"}/var/lib"
          }"
          "--runstatedir=/var/run"
        ]
