...
) "You must specify at least one of buildClient, buildServer, or buildExtraTools."; let
...
in
  stdenv.mkDerivation rec {
    ...
    nativeBuildInputs = [
      ...
      (
        if buildClient || buildServerGui
        then qt6Packages.wrapQtAppsHook
        else qt6Packages.wrapQtAppsNoGuiHook
      )
    ];
