...
in
  {
    ...
    generateMelpa = lib.makeOverridable (
      {archiveJson ? defaultArchive}: let
        inherit (import ./libgenerated.nix lib self) melpaDerivation;
        super = (
          lib.listToAttrs (
            builtins.filter (s: s != null) (
              map (melpaDerivation variant) (
                if builtins.isList archiveJson
                then archiveJson
                else lib.importJSON archiveJson
              )
            )
          )
        );
          ...
          // {
            ...
            shm =
              ...
              (attrs: {
                propagatedUserEnvPkgs =
                  attrs.propagatedUserEnvPkgs or []
                  ++ [
            ...
            alectryon = super.alectryon.overrideAttrs (
              finalAttrs: previousAttrs: {
                ...
                preBuild =
                  if lib.versionOlder finalAttrs.version "20241006.1902"
                  then
            ...
            bpr = super.bpr.overrideAttrs (
              finalAttrs: previousAttrs: {
                ...
                preBuild =
                  if lib.versionOlder finalAttrs.version "20241013.1803"
                  then
                    previousAttrs.preBuild or ""
                    + "\n"
                    + ''
            ...
            codesearch = ignoreCompilationError (
              super.codesearch.overrideAttrs (
                finalAttrs: previousAttrs: {
                  patches =
                    if lib.versionOlder finalAttrs.version "20240827.805"
                    then
                      previousAttrs.patches or []
                      ++ [
                        (pkgs.fetchpatch {
            ...
            dap-mode = super.dap-mode.overrideAttrs (
              finalAttrs: previousAttrs: {
                # empty file causing native-compiler-error-empty-byte
                preBuild =
                  if lib.versionOlder finalAttrs.version "20250131.1624"
                  then
                    ''
                      rm --verbose dapui.el
                    ''
            ...
            frontside-javascript = super.frontside-javascript.overrideAttrs (
              finalAttrs: previousAttrs: {
                ...
                preBuild =
                  if lib.versionOlder finalAttrs.version "20240929.1858"
                  then
                    previousAttrs.preBuild or ""
                    + "\n"
                    + ''
            ...
            kanagawa-themes = super.kanagawa-themes.overrideAttrs (
              finalAttrs: previousAttrs: {
                patches =
                  if lib.versionOlder finalAttrs.version "20241015.2237"
                  then
                    previousAttrs.patches or []
                    ++ [
                      (pkgs.fetchpatch {
            ...
            org-noter = super.org-noter.overrideAttrs (
              finalAttrs: previousAttrs: {
                patches =
                  if lib.versionOlder finalAttrs.version "20240915.344"
                  then
                    previousAttrs.patches or []
                    ++ [
                      (pkgs.fetchpatch {
            ...
            org-projectile = super.org-projectile.overrideAttrs (
              finalAttrs: previousAttrs: {
                ...
                preBuild =
                  if lib.versionOlder finalAttrs.version "20240901.2041"
                  then
                    ''
