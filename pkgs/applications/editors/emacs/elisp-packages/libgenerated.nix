lib: self: let
  ...
  fetchers = lib.mapAttrs (_: fetcher: self.callPackage fetcher {}) {
    github = {fetchFromGitHub}: {repo ? null, ...}: {
      sha256,
      commit,
      ...
    }: let
    ...
    gitlab = {fetchFromGitLab}: {repo ? null, ...}: {
      sha256,
      commit,
      ...
    }: let
        ...
        self.callPackage (
          {
            ...
            melpaBuild {
              ...
              src =
                if (sha256 == null || broken)
                then null
                else fetchers.${fetcher} args sourceArgs;
              recipe =
                if commit == null
                then null
                else
