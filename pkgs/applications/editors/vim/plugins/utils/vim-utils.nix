...
let
  ...
  pluginToDrv = knownPlugins: plugin: let
    drv =
  ...
  vamDictToNames = x:
    ...
    then [x]
    else (lib.optional (x ? name) x.name) ++ (x.names or []);
      ...
      vim:
        ...
        // {
          ...
          customize = {
            ...
            executableName ?
              if lib.hasInfix "vim" name
              then lib.replaceStrings ["vim"] ["$exe"] name
              else "\${exe/vim/${lib.escapeShellArg name}}",
            ...
            ''
              ...
              ${
                if wrapManual == true && name != "vim"
                then "Set `standalone = false` to include the manual."
                else
                  lib.optionalString (
            ...
            (
              let
                vimrc =
                  if vimrcFile != null
                  then vimrcFile
                  else if vimrcConfig != null
                  then mkVimrcFile vimrcConfig
                ...
                bin = runCommand "${name}-bin" {nativeBuildInputs = [makeWrapper];} ''
                  ...
                  for exe in ${
                    if standalone
                    then "{,g,r,rg,e}vim {,g}vimdiff vi"
