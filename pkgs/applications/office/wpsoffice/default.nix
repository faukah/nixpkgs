{
  ...
  url =
    ...
    then "https://wps-linux-personal.wpscdn.cn/wps/download/ep/Linux2019/${lib.last (lib.splitVersion pkgVersion)}/wps-office_${pkgVersion}_amd64.deb"
  ...
  hash =
    if useChineseVersion
    then "sha256-vpXK8YyjqhFdmtajO6ZotYACpe5thMct9hwUT3advUM="
    else "sha256-o8njvwE/UsQpPuLyChxGAZ4euvwfuaHxs5pfUvcM7kI=";
...
in
  stdenv.mkDerivation rec {
    ...
    src =
      ...
      (
        if useChineseVersion
        then "wps-office_${version}_amd64.deb"
        else "wps-office_${version}.XA_amd64.deb"
      )
      {
