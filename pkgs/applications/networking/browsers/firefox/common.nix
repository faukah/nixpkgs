...
in
  {
    ...
    assert elfhackSupport -> isElfhackPlatform stdenv; let
      ...
      buildStdenv = overrideCC llvmPackages.stdenv (
        llvmPackages.stdenv.cc.override {
          bintools =
            if ltoSupport
            then buildPackages.rustc.llvmPackages.bintools
            else stdenv.cc.bintools;
        }
      );
    ...
    in
      buildStdenv.mkDerivation {
        ...
        buildInputs =
          ...
          ++ lib.optionals stdenv.hostPlatform.isDarwin [
            (
              if lib.versionAtLeast version "138"
              then apple-sdk_15
              else apple-sdk_14
            )
            cups
          ...
          ++ (lib.optionals (!stdenv.hostPlatform.isDarwin) (
            [
              ...
              zlib
              (
                if (lib.versionAtLeast version "129")
                then nss_latest
                else nss_esr # 3.90
              )
            ]
