...
assert lib.assertMsg (!(sanitizeAddress && sanitizeThreads)) "'sanitizeAddress' and 'sanitizeThreads' are mutually exclusive, use one.";
assert testing -> !stable -> throw "testing implies stable and cannot be used with stable = false"; let
...
in
  stdenv.mkDerivation rec {
    ...
    version =
      if stable
      then kicadVersion
      else builtins.substring 0 10 src.rev;
