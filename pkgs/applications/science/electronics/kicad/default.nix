...
let
  ...
  kicadSrc =
    if srcOverridep "kicad"
    then srcs.kicad
    else kicadSrcFetch;
  kicadVersion =
    if srcOverridep "kicadVersion"
    then srcs.kicadVersion
    else versionsImport.${baseName}.kicadVersion.version;

  libSrc = name:
    if srcOverridep name
    then srcs.${name}
    else libSrcFetch name;
  # TODO does it make sense to only have one version for all libs?
  libVersion =
    if srcOverridep "libVersion"
    then srcs.libVersion
...
in
  stdenv.mkDerivation rec {
    ...
    version =
      if stable
      then kicadVersion
      else builtins.substring 0 10 src.src.rev;
    ...
    makeWrapperArgs = with passthru.libraries;
      ...
      ++ optionals (addons != []) (
        ...
        in ["--set-default NIX_KICAD9_STOCK_DATA_PATH ${stockDataPath}"]
      )
      ++ optionals with3d [
    ...
    meta = {
      description =
        (
          ...
          then "Open Source EDA suite, latest on stable branch"
