...
assert svnSupport -> perlSupport; let
  version = "2.49.0";
  ...
  stdenv.mkDerivation (finalAttrs: {
    ...
    src = fetchurl {
      url =
        ...
        then "https://www.kernel.org/pub/software/scm/git/testing/git-${
    ...
    makeFlags =
      ...
      ++ (
        if perlSupport
        then ["PERL_PATH=${perlPackages.perl}/bin/perl"]
        else ["NO_PERL=1"]
      )
      ++ (
        if pythonSupport
        then ["PYTHON_PATH=${python3}/bin/python"]
        else ["NO_PYTHON=1"]
      )
