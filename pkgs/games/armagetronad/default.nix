{
  ...
  in {
    ...
    "${latestVersionMajor}-sty+ct+ap" = let
  ...
  mkArmagetron = fn: dedicatedServer: let
    ...
    minorVersionPart = parts: sep: expectedSize:
      if builtins.length parts > expectedSize
      then sep + (builtins.concatStringsSep sep (lib.lists.drop expectedSize parts))
      else "";
  ...
  in
    stdenv.mkDerivation {
      ...
      passthru =
        ...
        else if (resolvedParams.version != (srcs.${latestVersionMajor} dedicatedServer).version)
        then {
