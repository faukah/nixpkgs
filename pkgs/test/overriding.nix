  ...
  in {
    ...
    overriding-using-only-attrset-no-final-attrs = {
      ...
      expr =
        ...
        == "hello-no-final-attrs-overridden";
      expected = true;
    };
  };

  test-extendMkDerivation = let
  ...
  stdenvNoCC.mkDerivation {
    ...
    buildCommand =
      ...
      + lib.concatMapAttrsStringSep "\n" (
        name: t: "([[ ${lib.boolToString t.expr} == ${lib.boolToString t.expected} ]] && echo '${name} success') || (echo '${name} fail' && exit 1)"
      )
      tests;
  }
