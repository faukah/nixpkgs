...
in
  {
    ...
    assert stdenv.hostPlatform.isWasm -> enableStaticLibraries == false; let
      inherit
        (lib)
        optional
        optionals
      ...
      allPkgconfigDepends = let
        ...
        propagateValue = drv: lib.isDerivation drv && (__onlyPropagateKnownPkgConfigModules -> drv ? meta.pkgConfigModules);
      ...
      nativeBuildInputs =
        ...
        ++ optional (allPkgconfigDepends != []) (
          assert pkg-config != null; pkg-config
        )
        ++ setupHaskellDepends
        ++ collectedToolDepends
          ...
          stdenv.mkDerivation (
            {
              ...
              ${
                if doInstallIntermediates
                then "installIntermediatesPhase"
                else null
              } = ''
                runHook preInstallIntermediates
                intermediatesOutput=${
                  if enableSeparateIntermediatesOutput
                  then "$intermediates"
                  else "$out"
                }
