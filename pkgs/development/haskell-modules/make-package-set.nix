...
self: let
  ...
  callPackageWithScope = scope: fn: manualArgs: let
...
in
  ...
  // {
    ...
    callCabal2nixWithOptions = name: src: opts: args: let
      extraCabal2nixOptions =
        if builtins.isString opts
        then opts
