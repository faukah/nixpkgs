{
  ...
  */
  ...
  phraseDirective = solution: env: name: val:
    ...
    then "${name}:${semicolons (map lib.escapeShellArg val)}"
  ...
  relSafeBinPath = input:
    ...
    then ((lib.getOutput "bin" input) + "/bin")
  ...
  phraseEnvVal = solution: env: val:
    if env == "inputs"
    then (colons (map relSafeBinPath val))
    else if builtins.isString val
    then val
    else if builtins.isList val
    then spaces val
    else if builtins.isAttrs val
    then spaces (phraseDirectives solution env val)
