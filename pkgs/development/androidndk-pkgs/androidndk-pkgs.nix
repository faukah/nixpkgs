{
  ...
  ndkBuildInfoFun = {config, ...}:
    ...
    .${
      config
    } or (throw "Android NDK doesn't support building on ${config}, as far as we know");
  ...
  ndkTargetInfoFun = {config, ...}:
    ...
    .${
      config
    } or (throw "Android NDK doesn't support targetting ${config}, as far as we know");

  buildInfo = ndkBuildInfoFun stdenv.buildPlatform;
  targetInfo = ndkTargetInfoFun stdenv.targetPlatform;

  androidSdkVersion =
    if (stdenv.targetPlatform ? androidSdkVersion && stdenv.targetPlatform.androidSdkVersion != null)
    then stdenv.targetPlatform.androidSdkVersion
    else (throw "`androidSdkVersion` is not set during the importing of nixpkgs");
