...
assert enableNumpy -> enablePython; let
  ...
  b2Args = lib.concatStringsSep " " (
    ...
    ++ lib.optionals (lib.versionAtLeast version "1.85") [
      (
        ...
        then "boost.stacktrace.from_exception=off"
        else "define=BOOST_STACKTRACE_LIBCXX_RUNTIME_MAY_CAUSE_MEMORY_LEAK"
      )
    ]
    ...
    [
      ...
      "architecture=${
        if stdenv.hostPlatform.isMips64
        then
          if lib.versionOlder version "1.78"
          then "mips1"
          else "mips"
      ...
      "binary-format=${
        if stdenv.hostPlatform.isMacho
        then "mach-o"
      ...
      "abi=${
        if stdenv.hostPlatform.parsed.cpu.family == "arm"
        then "aapcs"
        else if stdenv.hostPlatform.isWindows
        then "ms"
        else if stdenv.hostPlatform.isMips32
        then "o32"
        else if stdenv.hostPlatform.isMips64n64
        then "n64"
        else "sysv"
      }"
    ]
