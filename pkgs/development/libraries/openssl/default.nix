...
let
  common = {
    ...
    stdenv.mkDerivation (finalAttrs: {
      ...
      src = fetchurl {
        url =
          if lib.versionOlder version "3.0"
          then let
            versionFixed = builtins.replaceStrings ["."] ["_"] version;
          in "https://github.com/openssl/openssl/releases/download/OpenSSL_${versionFixed}/openssl-${version}.tar.gz"
          else "https://github.com/openssl/openssl/releases/download/openssl-${version}/openssl-${version}.tar.gz";
        inherit hash;
      };
      ...
      configureScript =
        {
          ...
          riscv32-linux = "./Configure ${
            if lib.versionAtLeast version "3.2"
            then "linux32-riscv32"
            else "linux-latomic"
          }";
        ...
        .${
          ...
          then
            ...
            else if stdenv.hostPlatform.isMicroBlaze
            then "./Configure linux-latomic"
            else if stdenv.hostPlatform.isMips32
            then "./Configure linux-mips32"
...
in {
  ...
  openssl_3_4 = common {
    ...
    patches = [
      ...
      (
        if stdenv.hostPlatform.isDarwin
        then ./3.4/use-etc-ssl-certs-darwin.patch
        else ./3.4/use-etc-ssl-certs.patch
      )
    ];
