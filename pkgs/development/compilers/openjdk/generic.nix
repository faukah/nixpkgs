    ...
    stdenv.mkDerivation (finalAttrs: {
      ...
      patches =
        [
          (
            if atLeast24
            then ./24/patches/fix-java-home-jdk24.patch
            else if atLeast21
            then ./21/patches/fix-java-home-jdk21.patch
            else if atLeast11
            then ./11/patches/fix-java-home-jdk10.patch
            else ./8/patches/fix-java-home-jdk8.patch
          )
          (
            if atLeast24
            then ./24/patches/read-truststore-from-env-jdk24.patch
            else if atLeast11
            then ./11/patches/read-truststore-from-env-jdk10.patch
            else ./8/patches/read-truststore-from-env-jdk8.patch
          )
        ]
        ++ lib.optionals (!atLeast23) [
          (
            if atLeast11
            then ./11/patches/currency-date-range-jdk10.patch
            else ./8/patches/currency-date-range-jdk8.patch
          )
        ]
        ++ lib.optionals atLeast11 [
          (
            if atLeast17
            then ./17/patches/increase-javadoc-heap-jdk13.patch
            else ./11/patches/increase-javadoc-heap.patch
          )
        ]
        ++ lib.optionals atLeast17 [
          (
            if atLeast21
            then ./21/patches/ignore-LegalNoticeFilePlugin-jdk18.patch
            else ./17/patches/ignore-LegalNoticeFilePlugin-jdk17.patch
          )
        ]
        ++ lib.optionals (!atLeast21) [
          (
            if atLeast17
            then ./17/patches/fix-library-path-jdk17.patch
            else if atLeast11
            then ./11/patches/fix-library-path-jdk11.patch
            else ./8/patches/fix-library-path-jdk8.patch
          )
        ]
        ...
        ++ lib.optionals (!headless && enableGtk) [
          (
            if atLeast17
            then ./17/patches/swing-use-gtk-jdk13.patch
            else if atLeast11
            then ./11/patches/swing-use-gtk-jdk10.patch
            else ./8/patches/swing-use-gtk-jdk8.patch
          )
        ];
      ...
      configureFlags =
        ...
        ++ lib.optional headless (
          if atLeast11
          then "--enable-headless-only"
          else "--disable-headful"
        )
      ...
      ${
        if atLeast23
        then "versionCheckProgram"
        else null
      } = "${placeholder "out"}/bin/java";
      ...
      ${
        ...
        false; # fails with "No rule to make target 'y'."
      ...
      installPhase =
        ...
        + lib.optionalString (!atLeast11) ''
          ...
          (
            ...
            then "$jre/lib/openjdk/jre/bin/keytool"
            else "keytool"
