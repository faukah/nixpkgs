...
in {
  ...
  gerbilVersionFromGit = srcDir: version-path: let
    version-file = "${srcDir}/${version-path}.ss";
  in
    if pathExists version-file
    then let
    ...
    in {
      ...
      in
        ...
        then commitIdFromGitRepo gitpath
        else "0";
    };
  ...
  gerbilSourceFilter = path: type: let
  ...
  sha256-of-pre-src = pre-src:
    if pre-src ? sha256
    then pre-src.sha256
    else "none";

  overrideSrcIfShaDiff = name: new-pre-src: super: let
