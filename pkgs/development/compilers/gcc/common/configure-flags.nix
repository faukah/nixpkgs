...
# ref: https://stackoverflow.com/questions/15670169/what-is-difference-between-sjlj-vs-dwarf-vs-seh
  let
    ...
    crossConfigureFlags =
      ...
      [
        "--with-as=${
          if targetPackages.stdenv.cc.bintools.isLLVM
          then binutils
          else targetPackages.stdenv.cc.bintools
        }/bin/${targetPlatform.config}-as"
        "--with-ld=${targetPackages.stdenv.cc.bintools}/bin/${targetPlatform.config}-ld"
      ]
      ++ (
        if withoutTargetLibc
        then
          [
            "--disable-libssp"
            "--disable-nls"
          ...
          [
            (
              if crossDarwin
              then "--with-sysroot=${lib.getLib libcCross}/share/sysroot"
              else "--with-headers=${lib.getDev libcCross}${libcCross.incdir or "/include"}"
            )
            "--enable-__cxa_atexit"
            "--enable-long-long"
            "--enable-threads=${
              if targetPlatform.isUnix
              then "posix"
              else if targetPlatform.isWindows
              then (threadsCross.model or "win32")
              else "single"
            }"
            "--enable-nls"
          ]
        ...
        (
          ...
          then
            (
              ...
              then "--with-native-system-header-dir=/usr/include"
