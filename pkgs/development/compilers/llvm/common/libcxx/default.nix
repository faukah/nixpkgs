...
assert cxxabi == null || !stdenv.hostPlatform.isDarwin; let
  cxxabiName = "lib${
    if cxxabi == null
    then "cxxabi"
    else cxxabi.libName
  ...
  cxxabiCMakeFlags =
    ...
    ++ lib.optionals (useLLVM && !stdenv.hostPlatform.isWasm) (
      if lib.versionAtLeast release_version "18"
      then [
...
in
  stdenv.mkDerivation (
    finalAttrs:
      ...
      // (
        if (lib.versionOlder release_version "16" || lib.versionAtLeast release_version "17")
        then {
          postPatch =
            (
              lib.optionalString
