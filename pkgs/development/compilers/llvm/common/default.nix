...
assert lib.assertMsg (lib.xor (gitRelease != null) (officialRelease != null)) (
  ...
  metadata = rec {
    ...
    versionDir =
      ...
      + "/${
        if (gitRelease != null)
        then "git"
        else lib.versions.major release_version
      }";
    getVersionFile = p:
      builtins.path {
        ...
        path = let
          ...
          patchDir =
            ...
            (
              if constraints == null
              then {path = metadata.versionDir;}
              else (lib.findFirst matchConstraint {path = metadata.versionDir;} constraints)
            ).path;
        in "${patchDir}/${p}";
      };
  };
  ...
  tools = lib.makeExtensible (
    tools: let
      ...
      clangVersion =
        if (lib.versionOlder metadata.release_version "16")
        then metadata.release_version
        else lib.versions.major metadata.release_version;
    ...
    in
      {
        ...
        clang =
          ...
          else if (pkgs.targetPackages.stdenv or args.stdenv).cc.isGNU
    ...
    in (
      {
        compiler-rt-libc = callPackage ./compiler-rt (
          let
            ...
            stdenv =
              ...
              if args.stdenv.hostPlatform.isDarwin
              then overrideCC darwin.bootstrapStdenv buildLlvmTools.clangWithLibcAndBasicRtAndLibcxx
              else if args.stdenv.hostPlatform.useLLVM or false
              then overrideCC args.stdenv buildLlvmTools.clangWithLibcAndBasicRtAndLibcxx
              else args.stdenv;
          in
            {
              inherit stdenv;
        ...
        compiler-rt-no-libc = callPackage ./compiler-rt {
          ...
          stdenv =
            ...
            then overrideCC darwin.bootstrapStdenv buildLlvmTools.clangNoLibcNoRt
            else overrideCC stdenv buildLlvmTools.clangNoLibcNoRt;
        };
        ...
        compiler-rt =
          ...
          then libraries.compiler-rt-no-libc
          else libraries.compiler-rt-libc;
        ...
        libcxx = callPackage ./libcxx (
          {
            stdenv =
              if stdenv.hostPlatform.isDarwin
              then overrideCC darwin.bootstrapStdenv buildLlvmTools.clangWithLibcAndBasicRt
      ...
      // lib.optionalAttrs (lib.versionAtLeast metadata.release_version "20") {
        ...
        libc-full = callPackage ./libc {
          ...
          cmake =
            if stdenv.targetPlatform.libc == "llvm"
            then buildPackages.cmakeMinimal
            else buildPackages.cmake;
          python3 =
            if stdenv.targetPlatform.libc == "llvm"
            then buildPackages.python3Minimal
            else buildPackages.python3;
        };

        libc =
          if stdenv.targetPlatform.libc == "llvm"
          then libraries.libc-full
          else libraries.libc-overlay;
      }
    )
  );
