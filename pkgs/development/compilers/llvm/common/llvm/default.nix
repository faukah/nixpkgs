...
in
  stdenv.mkDerivation (
    finalAttrs: let
      ...
      python =
        if finalAttrs.finalPackage.doCheck && !isDarwinBootstrap
        then
    ...
    in
      {
        ...
        postPatch =
          optionalString stdenv.hostPlatform.isDarwin (
            ...
            (optionalString (lib.versionAtLeast release_version "15") (
              ...
              (
                if lib.versionAtLeast release_version "16"
                then ''
        ...
        postInstall =
          ...
          + optionalString (stdenv.hostPlatform.isDarwin && enableSharedLibraries) ''
            ln -s $lib/lib/libLLVM.dylib $lib/lib/libLLVM-${
              if lib.versionOlder release_version "18"
              then "$shortVersion"
        ...
        doCheck =
          ...
          && (
            if lib.versionOlder release_version "15"
            then stdenv.hostPlatform.isLinux
            else true
          )
          && (
