...
in
  buildDunePackage rec {
    ...
    buildInputs =
      if lib.versionAtLeast version "1.12.0"
      then [
        pp
        re
        ppx_yojson_conv_lib
      ...
      else if lib.versionAtLeast version "1.10.0"
      then [
        pp
        re
        ppx_yojson_conv_lib
      ...
      else if lib.versionAtLeast version "1.7.0"
      then [
        re
        octavius
        dune-build-info
    ...
    propagatedBuildInputs =
      if lib.versionAtLeast version "1.14.0"
      then [
        jsonrpc
        ppx_yojson_conv_lib
        uutf
