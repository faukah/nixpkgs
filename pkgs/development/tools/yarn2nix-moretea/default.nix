{
  ...
  getLicenseFromSpdxId = licstr:
    if licstr == "UNLICENSED"
    then lib.licenses.unfree
    else lib.getLicenseFromSpdxId licstr;
in
  rec {
    ...
    # the package source.
    importOfflineCache = yarnNix: let
    ...
    mkYarnWorkspace = {
      ...
      packageGlobs =
        if lib.isList package.workspaces
        then package.workspaces
        else package.workspaces.packages;
      ...
      expandGlobList = base: globElems: let
      ...
      packages = lib.listToAttrs (
        map (
          src: let
            ...
            getWorkspaceDependencies = packages: allDependencies: let
