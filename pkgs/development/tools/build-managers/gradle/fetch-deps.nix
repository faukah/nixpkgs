  ...
  # the derivation to fetch/update deps for
  {
    ...
    data' =
      ...
      (
        ...
        else if builtins.isString data
        then lib.importJSON "${dirOf pkg.meta.position}/${data}"
        else data
      )
      [
        "!comment"
    ...
    decompressNameVer = prefix: let
      ...
      inherit
        (extractHashArtifact (lib.last splitHash))
        artifactId
        baseVer
        version
        clsSuf
        ;
    in
      if builtins.length splitHash == 1
      then builtins.head splitHash
      else
        ...
        ${url} =
          ...
          then {hash = v;}
            ...
            text = let
              ...
              uniqueVersionFiles = builtins.map ({
                i,
                x,
              }:
                x) (
                builtins.filter (
                  {
                    ...
                    i == 0 || (builtins.elemAt sortedJarPomList (i - 1)).version != x.version
              ...
              lastUpdated =
                v.lastUpdated
                      or (
                  if vMeta
                  then builtins.replaceStrings ["."] [""] snapshotTs
                  else "20240101123456"
                );
