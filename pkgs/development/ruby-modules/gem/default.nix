...
lib.makeOverridable (
  {
    ...
    src =
      attrs.src or (
        ...
        then fetchurl attrs.source
        else throw "buildRubyGem: don't know how to build a gem of type \"${type}\""
      );
    ...
    suffix =
      if type == "git"
      then builtins.substring 0 12 attrs.source.rev
  ...
  in
    stdenv.mkDerivation (
      ...
      // {
        ...
        env =
          lib.optionalAttrs (attrs ? env) attrs.env
          // {
