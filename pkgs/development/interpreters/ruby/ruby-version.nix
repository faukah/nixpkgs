...
{lib}: let
  ...
  rubyVersion = major: minor: tiny: tail: rec {
    ...
    in
      if lib.hasPrefix "p" tail && isPosInt p
      then p
      else null;
    ...
    # consistent.
    libDir =
      if lib.versionAtLeast majMinTiny "2.1.0"
      then "${majMin}.0"
      else if lib.versionAtLeast majMinTiny "2.0.0"
      then "2.0.0"
      else if lib.versionAtLeast majMinTiny "1.9.1"
      then "1.9.1"
      else throw "version ${majMinTiny} is not supported";
    ...
    __toString = self:
      ...
      + (
        ...
        then "-p${self.patchLevel}"
        else lib.optionalString (self.tail != "") "-${self.tail}"
      );
  };
in
  rubyVersion
