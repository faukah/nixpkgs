...
# files.
assert (enableCrypt -> (libxcrypt != null)); let
  crossCompiling = !(stdenv.buildPlatform.canExecute stdenv.hostPlatform);
  libc =
    if stdenv.cc.libc or null != null
    then stdenv.cc.libc
    else "/usr";
      ...
      in
        passthruFun rec {
          ...
          perlOnTargetForTarget =
            if lib.hasAttr perlAttr pkgsTargetTarget
            then (override pkgsTargetTarget.${perlAttr})
            else {};
        };
      ...
      postInstall =
        ''
          ...
          substituteInPlace "$out"/lib/perl5/*/*/Config_heavy.pl \
            ...
            --replace "${
            if stdenv.hasCC && stdenv.cc.cc != null
            then stdenv.cc.cc
            else "/no-such-path"
          }" /no-such-path \
            --replace "${
            if stdenv.hasCC && stdenv.cc.fallback_sdk or null != null
            then stdenv.cc.fallback_sdk
            else "/no-such-path"
          }" /no-such-path \
