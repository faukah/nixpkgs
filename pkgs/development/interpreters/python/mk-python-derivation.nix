# Generic builder.
{
  ...
  getOptionalAttrs = names: attrs: lib.getAttrs (lib.intersectLists names (lib.attrNames attrs)) attrs;
...
in
  {
    ...
    self = stdenv.mkDerivation (
      finalAttrs: let
        format' = assert (pyproject != null) -> (format == null);
          if pyproject != null
          then
            if pyproject
            then "pyproject"
        ...
        validatePythonMatches = let
          ...
          checkDrv = attrName: drv:
            ...
            then throwMismatch attrName drv
            else drv;
        in
          ...
          nativeBuildInputs =
            ...
            ++ optionals (format' == "pyproject") [
              ...
              (
                ...
                else pythonRuntimeDepsCheckHook
              )
            ]
