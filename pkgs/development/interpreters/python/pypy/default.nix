...
assert zlibSupport -> zlib != null; let
  ...
  passthru = passthruFun rec {
    ...
    executable = "pypy${
      if isPy39OrNewer
      then lib.versions.majorMinor pythonVersion
      else lib.optionalString isPy3k "3"
    }";
  ...
  with passthru;
    stdenv.mkDerivation rec {
      ...
      NIX_CFLAGS_COMPILE =
        ...
        then "-Wno-error=incompatible-function-pointer-types"
        else null;
      ...
      postPatch = ''
        ...
        substituteInPlace "lib-python/${
          if isPy3k
          then "3/tkinter/tix.py"
          else "2.7/lib-tk/Tix.py"
        }" \
      ...
      installPhase = ''
        ...
        cp ${../sitecustomize.py} $out/${
          if isPy38OrNewer
          then sitePackages
          else "lib/${libPrefix}/${sitePackages}"
        }/sitecustomize.py
