...
in rec {
  ...
  overrideLibcxx = stdenv:
    ...
    assert lib.versionAtLeast stdenv.cc.libcxx.version "12"; let
    ...
    stdenv.override (old: {
      mkDerivationFromStdenv = withOldMkDerivation old (
        stdenv: mkDerivationSuper: args:
          (mkDerivationSuper args).overrideAttrs (
            prevAttrs:
              if prevAttrs ? env.NIX_CFLAGS_LINK
              then {
                env =
                  prevAttrs.env
                  // {
  ...
  */
  useLibsFrom = modelStdenv: targetStdenv: let
