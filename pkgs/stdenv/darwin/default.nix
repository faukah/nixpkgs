...
# the bootstrap process and working with it.
{
  ...
  bootstrapFiles ?
    (config.replaceBootstrapFiles or lib.id) (
      if localSystem.isAarch64
      then import ./bootstrap-files/aarch64-apple-darwin.nix
      else import ./bootstrap-files/x86_64-apple-darwin.nix
    ),
}:
assert crossSystem == localSystem; let
  ...
  stageFun = prevStage: {
    ...
    cc =
      if prevStage.llvmPackages.clang-unwrapped == null
      then null
      else
            ...
            stdenv = import ../generic {
              ...
              overrides = self: super:
                mergeDisjointAttrs [
                  ...
                  {
                    "llvmPackages_${lib.versions.major prevStage.llvmPackages.release_version}" = let
