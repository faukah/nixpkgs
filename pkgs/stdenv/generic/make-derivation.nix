{
  ...
  */
  ...
  makeDerivationExtensible = rattrs: let
  ...
  makeDerivationArgument =
    ...
    # Policy on acceptable hash types in nixpkgs
      ...
      -> (
        ...
        in
          if algo == "md5"
          then throw "Rejected insecure ${algo} hash '${attrs.outputHash}'"
          else true
      ); let
        ...
        hardeningDisable' =
          if any (x: x == "fortify") hardeningDisable
        ...
        checkDependencyList' = positions: name: deps:
          imap1 (
            index: dep:
              if dep == null || isDerivation dep || builtins.isString dep || builtins.isPath dep
              then dep
              else if isList dep
              then checkDependencyList' ([index] ++ positions) name dep
              else
              ...
              in
                lib.strings.sanitizeDerivationName (
                  ...
                  then attrs.name + hostSuffix
                    ...
                    ) "The `version` attribute cannot be null."; "${attrs.pname}${staticMarker}${hostSuffix}-${attrs.version}"
                );
            })
            // {
  ...
  # Policy on acceptable hash types in nixpkgs
    ...
    -> (
      ...
      checkedEnv = let
        ...
        makeError = name: "  - ${name}: in `env`: ${prettyPrint env.${name}}; in derivation arguments: ${
